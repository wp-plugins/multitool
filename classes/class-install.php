...
class MULTITOOL_Install {
    ...
    */
    public function __construct() {
        ...
        // $this->DB = new MULTITOOL_DB(); commeted 14092014
        $this->DB = MULTITOOL::load_class( 'MULTITOOL_DB', 'class-wpdb.php', 'classes' );
        $this->PHP = new MULTITOOL_PHP();
                
        // on activation run install_plugin() method which then runs more methods i.e. create_tables();
        register_activation_hook( MULTITOOL_ABSPATH . 'multitool.php', array( $this, 'install_plugin' ) ); 

        // on deactivation run disabled_plugin() - not a full uninstall
        register_deactivation_hook( MULTITOOL_ABSPATH . 'multitool.php',  array( $this, 'deactivate_plugin' ) );
    ...
    */
    function create_tables() {       
        global $charset_collate, $wpdb, $MULTITOOL;
    ...
    */
    public function reinstalldatabasetables() {
        ...
        require_once( MULTITOOL_ABSPATH . 'arrays/tableschema_array.php' );
        
        if(is_array( $multitool_tables_array ) ){
            foreach( $multitool_tables_array['tables'] as $key => $table){
    ...
    function install_options() {
        ...
        update_option( 'multitool_installedversion', MULTITOOL::version );# will only be updated when user prompted to upgrade rather than activation
        update_option( 'multitool_installeddate',time() );# update the installed date, this includes the installed date of new versions
        
        // schedule settings
        require( MULTITOOL_ABSPATH . 'arrays/schedule_array.php' );        
        add_option( 'multitool_schedule', serialize( $multitool_schedule_array ) );

        // notifications array (persistent notice feature)
        add_option( 'multitool_notifications', serialize( array() ) ); 
    }
    ...
    function install_plugin() {              
        ...
        update_option( 'multitool_is_installed', true );
    } 
