...
class MULTITOOL {
    ...
    /**
     * Page hooks (i.e. names) WordPress uses for the MULTITOOL admin screens,
    ...
    */
    public function __construct() {
        ...
        $this->Install = self::load_class( 'MULTITOOL_Install', 'class-install.php', 'classes' );
        $this->Files = self::load_class( 'MULTITOOL_Files', 'class-files.php', 'classes' );
  
        $multitool_settings = self::adminsettings();
        ...
        if( is_admin() ){
            ...
            // admin globals 
            global $multitool_notice_array;
            
            $multitool_notice_array = array();// set notice array for storing new notices in (not persistent notices)
            
            // load class used from admin only                   
            $this->UI = self::load_class( 'MULTITOOL_UI', 'class-ui.php', 'classes' );
            $this->Helparray = self::load_class( 'MULTITOOL_Help', 'class-help.php', 'classes' );        
    ...
    */
    public function set_admin_globals() {
        global $multitool_menu_array;
        
        // set menu array
        $MULTITOOL_TabMenu = self::load_class( 'MULTITOOL_TabMenu', 'class-pluginmenu.php', 'classes' );
        $multitool_menu_array = $MULTITOOL_TabMenu->menu_array();   
        
        // set page name (it's my own approach, each tab/view has a name which is shorter than the WP view ID)
        $multitool_page_name = self::get_admin_page_name();             
    }
    ...
    */
    public function plugin_admin_register_styles() {
        wp_register_style( 'multitool_css_notification',plugins_url( 'multitool/css/notifications.css' ), array(), '1.0.0', 'screen' );
        wp_register_style( 'multitool_css_admin',plugins_url( 'multitool/css/admin.css' ), __FILE__);          
    }
    ...
    */
    public function plugin_admin_print_styles() {
        wp_enqueue_style( 'multitool_css_notification' );  
        wp_enqueue_style( 'multitool_css_admin' );               
    }    
    ...
    */
    ...
    public function enqueue_style( $name, array $dependencies = array() ) {
        ...
        $css_url = plugins_url( $css_file, MULTITOOL__FILE__ );
        wp_enqueue_style( "multitool-{$name}", $css_url, $dependencies, MULTITOOL::version );
    }
    ...
    public function enqueue_script( $name, array $dependencies = array(), $localize_script = false, $force_minified = false ) {
        ...
        $js_url = plugins_url( $js_file, MULTITOOL__FILE__ );
        wp_enqueue_script( "multitool-{$name}", $js_url, $dependencies, MULTITOOL::version, true );
    }  
    ...
    */
    public static function load_class( $class, $file, $folder, $params = null ) {
         ...
         */        
        $class = apply_filters( 'multitool_load_class_name', $class );
    ...
    /**
    * returns the MULTITOOL_WPMain class object already created in this MULTITOOL class
    * 
    * @author Ryan R. Bayne
    * @package Multitool
    * @since 0.0.1
    ...
    */
    ...
    public static function load_file( $file, $folder ) {      
        $full_path = MULTITOOL_ABSPATH . $folder . '/' . $file;
         ...
         */
        $full_path = apply_filters( 'multitool_load_file_full_path', $full_path, $file, $folder );
        if ( $full_path ) {   
    ...
    public static function load_draggableboxes_view( $page_slug, array $data = array() ) {
        ...
        require_once( MULTITOOL_ABSPATH . 'classes/class-view.php' );
        ...
        $the_view = self::load_class( "MULTITOOL_{$ucview}_View", "{$page_slug}.php", $dir );
     ...
     */
     public function load_admin_page() {
        // remove "multitool_" from page value in URL which leaves the page name as used in the menu array
        $page = 'main';
        if( isset( $_GET['page'] ) && $_GET['page'] !== 'multitool' ){    
            $page = substr( $_GET['page'], strlen( 'multitool_' ) );
        }
    ...
    */
    private function filteraction_should_beloaded( $whenToLoad) {
        $multitool_settings = $this->adminsettings();
        ...
        switch( $whenToLoad) {
            ...
            case 'pluginscreens':
                ...
                // load when on a Multitool admin screen
                if( isset( $_GET['page'] ) && strstr( $_GET['page'], 'multitool' ) ){return true;}
            ...
            case 'pluginanddashboard':
                ...
                if( isset( $_GET['page'] ) && strstr( $_GET['page'], 'multitool' ) ){
                    return true;
                }
            ...
            case 'systematicpostupdating':  
                if(!isset( $multitool_settings['standardsettings']['systematicpostupdating'] ) || $multitool_settings['standardsettings']['systematicpostupdating'] != 'enabled' ){
                    return false;    
                }      
                return true;
            break;
    ...
    */
    public function debugmode() {
        global $multitool_debug_mode;
        if( $multitool_debug_mode){
            global $wpdb;
    ...
    */
    ...
    /**
    * Gets option value for multitool _adminset or defaults to the file version of the array if option returns invalid.
    * 1. Called in the main multitool.php file.
    ...
    */
    public function adminsettings() {
        $result = $this->option( 'multitool_settings', 'get' );
    ...
    * 
    * @uses MULTITOOL_UI::add_dashboard_widgets() which has the widgets
    * 
    * @author Ryan R. Bayne
    * @package Multitool
    * @since 0.0.1
    ...
    */       
    ...
    public function screen_options() {
        ...
        // toplevel_page_multitool (main page)
        if( $screen->id == 'toplevel_page_multitool' ){
            $args = array(
                'label' => __( 'Members per page' ),
                'default' => 1,
                'option' => 'multitool_testoption'
            );
    ...
    public function save_screen_option( $status, $option, $value ) {
        if ( 'multitool_testoption' == $option ) return $value;
    }
    ...
    */
    public function help_tab () {
        ...
        $MULTITOOL_Help = self::load_class( 'MULTITOOL_Help', 'class-help.php', 'classes' );
        ...
        // get page name i.e. multitool_page_multitool_affiliates would return affiliates
        $page_name = $this->PHP->get_string_after_last_character( $screen->id, '_' );
        
        // if on main page "multitool" then set tab name as main
        if( $page_name == 'multitool' ){$page_name = 'main';}
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewreadmoreurl' ] ) ){
            $help_content .= '<p>';
            $help_content .= __( 'You are welcome to visit the', 'multitool' ) . ' ';
            $help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewreadmoreurl' ] . '"';
            $help_content .= 'title="' . __( 'Visit the Multitool website and read more about', 'multitool' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';
            $help_content .= __( 'Multitool Website', 'multitool' ) . '</a> ' . __( 'to read more about', 'multitool' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ];           
            $help_content .= '.</p>';
        }  
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewvideourl' ] ) ){
            $help_content .= '<p>';
            $help_content .= __( 'There is a', 'multitool' ) . ' ';
            $help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewvideourl' ] . '"';
            $help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'multitool' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';            
            $help_content .= __( 'YouTube Video', 'multitool' ) . '</a> ' . __( 'about', 'multitool' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ];           
            $help_content .= '.</p>';
        }
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewdiscussurl' ] ) ){
            $help_content .= '<p>';
            $help_content .= __( 'We invite you to take discuss', 'multitool' ) . ' ';
            $help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewdiscussurl' ] . '"';
            $help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'multitool' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';            
            $help_content .= $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'multitool' );           
            $help_content .= '.</p>';
        }         
        ...
        $screen->add_help_tab( array(
            'id'    => $page_name,
            'title'    => __( 'About', 'multitool' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] ,
        ...
        foreach( $help_array[ $page_name ][ $view_name ][ 'forms' ] as $form_id => $value ){
            ...
            if( isset( $value[ 'formreadmoreurl' ] ) ){
                ...
                $help_content .= 'title="' . __( 'Visit the Multitool website and read more about', 'multitool' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';
                $help_content .= __( 'Multitool Website', 'multitool' ) . '</a> ' . __( 'to read more about', 'multitool' ) . ' ' . $value[ 'formtitle' ];           
                $help_content .= '.</p>';
            }  
            ...
            if( isset( $value[ 'formvideourl' ] ) ){
                ...
                $help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'multitool' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';            
                $help_content .= __( 'YouTube Video', 'multitool' ) . '</a> ' . __( 'about', 'multitool' ) . ' ' . $value[ 'formtitle' ];           
                $help_content .= '.</p>';
            }
            ...
            if( isset( $value[ 'formdiscussurl' ] ) ){
                $help_content .= '<p>';
                $help_content .= __( 'We invite you to discuss', 'multitool' ) . ' ';
                $help_content .= '<a href="' . $value[ 'formdiscussurl' ] . '"';
                $help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'multitool' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';            
                $help_content .= $value[ 'formtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'multitool' );           
                $help_content .= '.</p>';
            } 
            ...
            foreach( $value[ 'options' ] as $key_two => $option_array ){  
                ...
                if( isset( $option_array['optionurl'] ) ){
                    ...
                    $help_content .= ' title="' . __( 'Read More about', 'multitool' )  . ' ' . $option_array['optiontitle'] . '"';
                    $help_content .= ' target="_blank">';
                    $help_content .= __( 'Read More', 'multitool' ) . '</a>';      
                }
      
                if( isset( $option_array['optionvideourl'] ) ){
                    $help_content .= ' - <a href="' . $option_array['optionvideourl'] . '"';
                    $help_content .= ' title="' . __( 'Watch a video about', 'multitool' )  . ' ' . $option_array['optiontitle'] . '"';
                    $help_content .= ' target="_blank">';
                    $help_content .= __( 'Video', 'multitool' ) . '</a>';      
                }
            }
    ...
    * @version 1.0
    *  
    * @param mixed $multitool_page_name
    * @param mixed $default
    */
    ...
    */
    public function admin_menu() {    
        ...
        $MULTITOOL_Menu = $multitool_menu_array;
        ...
        // add menu
        $this->page_hooks[] = add_menu_page( $MULTITOOL_Menu['main']['title'], 
        __( 'Multitool', 'multitool' ), 
        'administrator', 
        'multitool',  
        $subpage_callback ); 
        
        // help tab                                                 
        add_action( 'load-toplevel_page_multitool', array( $this, 'help_tab' ) );
        ...
        foreach( $MULTITOOL_Menu as $key_pagename => $page_array ){ 
        ...
        foreach( $MULTITOOL_Menu as $key_pagename => $page_array ){                 
            ...
            if( !isset( $_GET['page'] ) || !strstr( $_GET['page'], 'multitool' ) ){
                ...
                // remove none parents
                if( $page_array['parent'] !== 'parent' ){    
                    unset( $MULTITOOL_Menu[ $key_pagename ] ); 
                }        
            
            }elseif( isset( $_GET['page'] ) && strstr( $_GET['page'], 'multitool' ) ){
            ...
            if( isset( $_GET['page'] ) && $page_array['slug'] === $_GET['page'] ){
                unset( $MULTITOOL_Menu[ $MULTITOOL_Menu[ $key_pagename ]['parent'] ] );
            }
            
            // remove update page as it is only meant to show when new version of files applied
            if( $page_array['slug'] == 'multitool_pluginupdate' ) {
                unset( $MULTITOOL_Menu[ $key_pagename ] );
            }
        }
        ...
        foreach( $MULTITOOL_Menu as $key_pagename => $page_array ){ 
            ...
            $new_hook = add_submenu_page( 'multitool', 
                   $group_titles_array[ $page_array['groupname'] ]['grouptitle'], 
                   $group_titles_array[ $page_array['groupname'] ]['grouptitle'], 
                   self::get_page_capability( $key_pagename ), 
                   $MULTITOOL_Menu[ $key_pagename ]['slug'], 
                   $subpage_callback );     
         
            $this->page_hooks[] = $new_hook;
                   
            // help tab                                                 
            add_action( 'load-multitool_page_multitool_' . $key_pagename, array( $this, 'help_tab' ) );              
        }
    }
    ...
    * 
    * function was reduced to two lines, the contents mode to MULTITOOL_Requests itself.
    *
    * @author Ryan R. Bayne
    * @package Multitool
    * @since 0.0.1
    ...
    */
    public function process_admin_POST_GET() {
        ...
        if( isset( $_POST['multitool_admin_action'] ) ) {
            $early_deny_request = false;
                
            // run nonce security for a form submission
            if( !isset( $_POST['multitool_form_name'] ) ) {    
                return false;
            }

            check_admin_referer( $_POST['multitool_form_name'] );# exists here if failed
            $function = $_POST['multitool_form_name'];
        ...
        if( isset( $_GET['multitoolaction'] )  ) {
               
            check_admin_referer( $_GET['multitoolaction'] );# exists here if failed
            $function = $_GET['multitoolaction'];  
        ...
        if( $method !== 'unknown' ) {   
            $MULTITOOL_REQ = self::load_class( 'MULTITOOL_Requests', 'class-requests.php', 'classes' );
            $MULTITOOL_REQ->process_admin_request( $method, $function );
        }
    } 
    ...
    */
    public function pluginmediabutton_popup() {
        ...
        <div id="multitool_popup_container" style="display:none;">
    ...
    * 
    * Debugging Trace
    * $multitool_schedule_array['history']['trace'] is used to indicate how far the this script went before a return.
    ...
    */
    public function event_check() {
        $multitool_schedule_array = self::get_option_schedule_array();
        
        // do not continue if WordPress is DOING_AJAX
        if( self::request_made() ){return;}
                      
        self::log_schedule( __( 'The schedule is being checked. There should be further log entries explaining the outcome.', 'multitool' ), __( 'schedule being checked', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
        ...
        //  do not use multitool_event_refused as we do not want to set the array
        if(!isset( $multitool_schedule_array ) || !is_array( $multitool_schedule_array ) ){       
            self::log_schedule( __( 'Scheduled events cannot be peformed due to the schedule array of stored settings not existing.', 'multitool' ), __( 'schedule settings missing', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            return false;
        }
        ...
        if( isset( $multitool_schedule_array['history']['lasteventtime'] ) )
        {    
            // increase lasteventtime by 60 seconds
            $soonest = $multitool_schedule_array['history']['lasteventtime'] + 60;//hack info page http://www.webtechglobal.co.uk/hacking/increase-automatic-events-delay-time
            
            if( $soonest > time() ){
                self::log_schedule( __( 'No changed made as it has not been 60 seconds since the last event.', 'multitool' ), __( 'enforcing schedule event delay', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
                self::event_return( __( 'has not been 60 seconds since list event', 'multitool' ) ); 
                return;
            }             
        }
        else
        {               
            ...
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'The last even time event was set for the first time, no further processing was done.', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            ...
            changes are made during this schedule check.', 'multitool' ), __( 'initialized schedule delay timer', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);

            self::event_return( __( 'initialised the last event time value', 'multitool' ) );
            return;        
        }                             
                                           
        // is last event type value set? if not set default as dataupdate, this means postcreation is the next event
        if(!isset( $multitool_schedule_array['history']['lasteventtype'] ) )
        {    
            $multitool_schedule_array['history']['lasteventtype'] = 'dataupdate';
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'The last event type value was set for the first time', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            ...
            determine what event comes next.', 'multitool' ), __( 'initialized schedule last event value', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            
            self::event_return( __( 'initialised last event type value', 'multitool' ) );
            return;
        }
                 
        // does the "day_lastreset"" time value exist, if not we set it now then return
        if(!isset( $multitool_schedule_array['history']['day_lastreset'] ) )
        {    
            $multitool_schedule_array['history']['day_lastreset'] = time();
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'The last daily reset time was set for the first time', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            ...
            and should only happen once.', 'multitool' ), __( '24 hour timer set', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            
            self::event_return( __( 'initialised last daily reset time', 'multitool' ) );        
            return;
        } 
                                                         
        // does the "hour_lastreset"" time value exist, if not we set it now then return
        if(!isset( $multitool_schedule_array['history']['hour_lastreset'] ) )
        { 
            $multitool_schedule_array['history']['hour_lastreset'] = time();
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'The hourly reset time was set for the first time', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            ...
            taking. This should only happen once.', 'multitool' ), __( 'one hour timer set', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);        
            
            self::event_return( __( 'initialised hourly reset time', 'multitool' ) );
            return;
        }    
               
        // does the hourcounter value exist, if not we set it now then return (this is to initialize the variable)
        if(!isset( $multitool_schedule_array['history']['hourcounter'] ) )
        {     
            $multitool_schedule_array['history']['hourcounter'] = 0;
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'The hourly events counter was set for the first time', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            self::log_schedule( __( 'Number of events per hour has been set for the first time, this change is treated as an event.', 'multitool' ), __( 'hourly events counter set', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);     
            self::event_return( __( 'initialised hourly events counter', 'multitool' ) );   
            return;
        }     
                                     
        // does the daycounter value exist, if not we set it now then return (this is to initialize the variable)
        if(!isset( $multitool_schedule_array['history']['daycounter'] ) )
        {
            $multitool_schedule_array['history']['daycounter'] = 0;
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'The daily events counter was set for the first time', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            self::log_schedule( __( 'The daily events counter was not set. No further action was taking. This measure should only happen once.', 'multitool' ), __( 'daily events counter set', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);     
            self::event_return( __( 'initialised daily events counter', 'multitool' ) );           
            return;
        } 
        ...
        $hour_reset_time = $multitool_schedule_array['history']['hour_lastreset'] + 3600;
        if(time() > $hour_reset_time )
        {     
            // reset hour_lastreset value and the hourlycounter
            $multitool_schedule_array['history']['hour_lastreset'] = time();
            $multitool_schedule_array['history']['hourcounter'] = 0;
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'Hourly counter was reset for another 60 minute period', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            self::log_schedule( __( 'Hourly counter has been reset, no further action is taking during this event. This should only happen once every hour.', 'multitool' ), __( 'hourly counter reset', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'hourly counter was reset', 'multitool' ) );        
            return;
        }  
        ...
        $day_reset_time = $multitool_schedule_array['history']['day_lastreset'] + 86400;
        if(time() > $day_reset_time )
        {
            ...
            $multitool_schedule_array['history']['hourcounter'] = 0;
            $multitool_schedule_array['history']['daycounter'] = 0;
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'Daily and hourly events counter reset for a new 24 hours period', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array ); 
            self::log_schedule( __( '24 hours had passed and the daily counter had to be reset. No further action is taking during these events and this should only happen once a day.', 'multitool' ), __( 'daily counter reset', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);   
        ...
        if(!isset( $multitool_schedule_array['days'][$day] ) )
        {
            self::event_return( __( 'Event processing is has not been permitted for today', 'multitool' ) );
            self::log_schedule( __( 'Event processing is not permitted for today. Please check schedule settings to change this.', 'multitool' ), __( 'schedule not permitted today', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
        ...
        if(!isset( $multitool_schedule_array['hours'][$hour] ) )
        {
            self::event_return( __( 'Event processing is has not been permitted for the hour', 'multitool' ) );
            self::log_schedule( __( 'Processsing is not permitted for the current hour. Please check schedule settings to change this.', 'multitool' ), __( 'hour not permitted', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'schedule not permitting hour', 'multitool' ) );        
            return;    
        }

        // ensure hourly limit value has been set
        if(!isset( $multitool_schedule_array['limits']['hour'] ) )
        {  
            $multitool_schedule_array['limits']['hour'] = 1;
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'Hourly limit was set for the first time', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            self::log_schedule( __( 'The hourly limit value had not been set yet. You can change the limit but the default has been set to one. No further action is taking during this event and this should only happen once.', 'multitool' ), __( 'no hourly limit set', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'initialised hourly limit', 'multitool' ) );        
            return;
        }     
                    
        // ensure daily limit value has been set
        if(!isset( $multitool_schedule_array['limits']['day'] ) )
        {
            $multitool_schedule_array['limits']['day'] = 1;
            $multitool_schedule_array['history']['lastreturnreason'] = __( 'Daily limit was set for the first time', 'multitool' );
            self::update_option_schedule_array( $multitool_schedule_array );
            self::log_schedule( __( 'The daily limit value had not been set yet. It has now been set as one which allows only one post to be created or updated etc. This action should only happen once.', 'multitool' ), __( 'no daily limit set', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__); 
            self::event_return( __( 'initialised daily limit', 'multitool' ) );           
            return;
        }

        // if this hours target has been met return
        if( $multitool_schedule_array['history']['hourcounter'] >= $multitool_schedule_array['limits']['hour'] )
        {
            self::event_return( 'The hours event limit/target has been met' );
            self::log_schedule( __( 'The events target for the current hour has been met so no further processing is permitted.', 'multitool' ), __( 'hourly target met', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'hours limit reached', 'multitool' ) );            
            return;        
        }
         
        // if this days target has been met return
        if( $multitool_schedule_array['history']['daycounter'] >= $multitool_schedule_array['limits']['day'] )
        {
            self::event_return( __( 'The days event limit/target has been met', 'multitool' ) );
            self::log_schedule( __( 'The daily events target has been met for the current 24 hour period (see daily timer counter). No events will be processed until the daily timer reaches 24 hours and is reset.', 'multitool' ), __( 'daily target met', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            self::event_return( __( 'days limit reached', 'multitool' ) );        
            return;       
        }
        ...
        self::log_schedule(sprintf( __( 'The schedule system decided that the next event type is %s.', 'multitool' ), $run_event_type), __( 'next event type determined', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);
            
        // update $multitool_schedule_array with decided event type to advance the cycle and increase hourly plus daily counter
        $multitool_schedule_array['history']['lasteventtype'] = $run_event_type;
        $multitool_schedule_array['history']['lasteventtime'] = time(); 
        $multitool_schedule_array['history']['hourcounter'] = $multitool_schedule_array['history']['hourcounter'] + 1; 
        $multitool_schedule_array['history']['daycounter'] = $multitool_schedule_array['history']['daycounter'] + 1;
        self::update_option_schedule_array( $multitool_schedule_array );
    ...
    * multitool solution. Passing value to this method, can be used to call more specific method
    * for an event.
    * 
    * @author Ryan R. Bayne
    * @package Multitool
    ...
    * Must call ths function within a function that checks that uses multitool_DOING_AJAX() first 
    * 
    * 1. If you add a new event type, you must also update multitool_tab1_pagecreation.php (Schedule), specifically Events Status panel
    * 2. Update event_action when adding a new event type
    * 3. Update the Event Types panel and add option for new event types
    * 4. Update multitool_form_save_eventtypes
    * 
    * @link http://www.webtechglobal.co.uk/hacking/event-types
    */
    public function event_decide() {
        global $multitool_schedule_array, $multitool_settings;
        ...
        {
            self::log_schedule(sprintf( __( 'The plugins ability to override the next due event type has been applied and then next event forced is %s.', 'multitool' ), $override_event), __( 'next event type override', 'multitool' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);         
        ...
        if( $multitool_schedule_array['history']['lasteventtype'] == 'dataimport' ){
            if( isset( $multitool_schedule_array['eventtypes']['dataupdate']['switch'] ) && $multitool_schedule_array['eventtypes']['dataupdate']['switch'] == true){
                 return 'dataupdate';    
            }else{
                $bypass = true; 
            }            
        ...
        if( $multitool_schedule_array['history']['lasteventtype'] == 'dataupdate' || $bypass == true){
            if( isset( $multitool_schedule_array['eventtypes']['postcreation']['switch'] ) && $multitool_schedule_array['eventtypes']['postcreation']['switch'] == true){
                 return 'postcreation';    
            }else{
                $bypass = true; 
            }            
        ...
        if( $multitool_schedule_array['history']['lasteventtype'] == 'postcreation' || $bypass == true){
            if( isset( $multitool_schedule_array['eventtypes']['postupdate']['switch'] ) && $multitool_schedule_array['eventtypes']['postupdate']['switch'] == true){
                return 'postupdate';    
            }else{
                $bypass = true; 
            }            
        ...
        if( $multitool_schedule_array['history']['lasteventtype'] == 'postupdate' || $bypass == true){
            if( isset( $multitool_schedule_array['eventtypes']['dataimport']['switch'] ) && $multitool_schedule_array['eventtypes']['dataimport']['switch'] == true){
                 return 'dataimport';    
            }else{
                $bypass = true; 
            }            
    ...
    */
    public function event_decide_focus() {
        $multitool_schedule_array = self::get_option_schedule_array();
        if( isset( $multitool_schedule_array['focus'] ) && $multitool_schedule_array['focus'] != false ){
            return $multitool_schedule_array['focus'];    
        }
    }
    ...
    */
    public function event_action( $run_event_type){    
        global $multitool_settings, $MULTITOOL;
        $multitool_schedule_array = MULTITOOL::get_option_schedule_array();       
        $multitool_schedule_array['history']['lasteventaction'] = $run_event_type . ' Requested'; 
        ...
        switch ( $run_event_type) {
            ...
            case "postcreation":
                ...
                //multitool_event_return( 'post creation procedure complete' );
            ...
            case "postupdate":
                ...
                * 1st = Changed Records: search for records updated but not applied (post update function will update the multitool_applied or at least the sql function called will)
                * 2nd = Meta Key: multitool_outdated meta is searched, any meta value of 'yes' indicates posts needs to be updated 
                * 3rd = Changed Projected: project array includes a value ['updatecomplete'] which is set to true when no posts can be found with multitool_updated dates older than ['modified']. This allows an update to all posts. 
                */                                
                ...
                if( $record){
                    ...
                    $multitool_schedule_array['history']['lasteventaction'] = 'post with ID ' . $r['multitool_postid'] . ' was updated with a new record';        

                }
          
                ############################################################################
                #                                                                          #
                #             multitool_outdated META VALUE SET TO "yes" METHOD             #
                #                                                                          #
                ############################################################################         
                if(!$updating_carried_out){

                    // locate a post that have been marked as multitool_outdated
                    //$post = multitool_WP_SQL_get_posts_join_meta( 'multitool_outdated', 'yes', $limit = 1, $select = 'ID' );
                    if( $post){
                        ...
                        foreach( $post as $key => $p){
                            ...
                            $project_code = get_post_meta( $p->ID, 'multitool_project_code', true);
                            if(!$project_code){continue;}
                            
                            //multitool_post_updatepost( $p->ID,get_post_meta( $p->ID, 'multitool_record_id', true), $project_code);
             
                            //multitool_log_schedule( 'Post with ID '.$p->ID.' was updated because it was marked as outdated', 'post updated',1, $run_event_type, __LINE__, __FILE__, __FUNCTION__, 'medium' );
             
                            // update the lasteventaction value - a very short sentence explaining the specific changes in blog
                            $multitool_schedule_array['history']['lasteventaction'] = 'post ' . $p->ID . ' was updated using multitool_outdated method';        
                        }
                    }                  
                }
                ...
                // against the multitool_updated meta value  
                if(!$updating_carried_out){
                    ...
                    */
                    ...
                    // run query for a post that has an older multitool_last_updated date than the projects modified date                        
                    $post = multitool_SQL_get_posts_oudated( $project_modified, $project_code, $limit = 1, $select = 'ID' );
                    ...
                    if( $post){
                        ...
                        foreach( $post as $key => $p){
                            ...
                            $record_id = get_post_meta( $p->ID, 'multitool_record_id', true);
                            if( $record_id && is_numeric( $record_id) ){
                                
                                // update the post, this function does not just use new records
                                //multitool_post_updatepost( $p->ID, $record_id, $project_code);
             
                                // update the lasteventaction value - a very short sentence explaining the specific changes in blog
                                $multitool_schedule_array['history']['lasteventaction'] = 'post with ID ' . $p->ID . ' was updated because project configuration was changed';
                        ...
                        // update the lasteventaction value - a very short sentence explaining the specific changes in blog
                        $multitool_schedule_array['history']['lasteventaction'] = 'schedule determine posts for project '.$outdated_project_code.' are all updated';
                ...
                multitool_event_return( 'data update procedure finished' ); 
    ...
    */
    public function pluginmediabutton_button( $context) {
        //append the icon
        $context = "<a class='button thickbox' title='Multitool Column Replacement Tokens (CTRL + C then CTRL + V)'
        href='#TB_inline?width=400&inlineId=multitool_popup_container'>Multitool</a>";
        return $context;
    }  
    ...
    */
    public function install_admin_settings() {
        require_once( MULTITOOL_ABSPATH . 'arrays/settings_array.php' );
        return $this->option( 'multitool_settings', 'update', $multitool_settings );# update creates record if it does not exist   
    } 
    ...
    */
    public function custom_post_types() { 
        ...
        if( isset( $multitool_settings['posttypes']['wtgflags']['status'] ) && $multitool_settings['posttypes']['wtgflags']['status'] === 'enabled' ) {    
            require( MULTITOOL_ABSPATH . 'posttypes/flags.php' );   
        }
        
        // has post "Edit Post" enhancements been setup for this package and activated in default settings?                                                       
        if( isset( $multitool_settings['posttypes']['posts']['status'] ) && $multitool_settings['posttypes']['posts']['status'] === 'enabled' ) {    
            require( MULTITOOL_ABSPATH . 'posttypes/posts.php' );   
        }
    }
    ...
    */
    public function request_made() {
        ...
        // Multitool own special processing triggers
        if( isset( $_GET['multitoolaction'] ) || isset( $_GET['nonceaction'] ) ){
            return true;
        }
    ...
    * 
    * @link http://www.multitool.com/hacking/log-table
    */
    public function newlog( $atts ){     
        global $multitool_settings, $wpdb, $multitool_currentversion;
        ...
        if( isset( $multitool_settings['globalsettings']['uselog'] ) && $multitool_settings['globalsettings']['uselog'] == 0){
            return false;
        }
        ...
        extract( shortcode_atts( array(  
            ...
            'version' => $multitool_currentversion, 
    ...
    */
    ...
    function conflict_prevention( $outputnoneactive = false ){
        ...
        $plugin_profiles[0]['title'] = __( 'Tweet My Post', 'multitool' );
        $plugin_profiles[0]['slug'] = 'tweet-my-post/tweet-my-post.php';
        $plugin_profiles[0]['author'] = 'ksg91';
        $plugin_profiles[0]['title_active'] = __( 'Tweet My Post Conflict', 'multitool' );
        $plugin_profiles[0]['message_active'] = __( 'Please deactivate Twitter plugins before performing mass post creation. This will avoid spamming Twitter and causing more processing while creating posts.', 'multitool' );
        $plugin_profiles[0]['message_inactive'] = __( 'If you activate this or any Twitter plugin please ensure the plugins options are not setup to perform mass tweets during post creation.', 'multitool' );
    ...
    */
    ...
    public function send_email( $recipients, $subject, $content, $content_type = 'html' ){     
        ...
        {
            add_filter( 'wp_mail_content_type', 'multitool_set_html_content_type' );
        }
        ...
        if( $content_type == 'html' )
        {    
            remove_filter( 'wp_mail_content_type', 'multitool_set_html_content_type' );  
        }   
    ...
    *  
    * @param mixed $page, registered page slug i.e. multitool_install which results in wp-admin/admin.php?page=multitool_install   
    ...
    */
    public function formend_standard( $buttontitle = 'Submit', $buttonid = 'notrequired' ){
            if( $buttonid == 'notrequired' ){
                $buttonid = 'multitool_notrequired'.rand(1000,1000000);# added during debug
            }else{
                $buttonid = $buttonid.'_formbutton';
            }?>

            <p class="submit">
                <input type="submit" name="multitool_wpsubmit" id="<?php echo $buttonid;?>" class="button button-primary" value="<?php echo $buttontitle;?>">
            </p>
    ...
    public function formstart_standard( $name, $id = 'none', $method = 'post', $class, $action = '', $enctype = '' ){
        ...
        echo '<form '.$class.' '.$enctype.' id="'.$id.'" method="'.$method.'" name="multitool_request_'.$name.'" action="'.$action.'">
        <input type="hidden" id="multitool_admin_action" name="multitool_admin_action" value="true">';
    } 
    ...
    */
    public function pageheader( $pagetitle, $layout ){
        global $current_user, $multitool_settings;
        ...
        <div id="multitool-page" class="wrap">
            ...
            // if not "Multitool" set this title
            if( $pagetitle !== 'Multitool' ) {
    ...
    */
    public function check_requirements( $display ){
        ...
        // php version
        if(defined(MULTITOOL_PHPVERSIONMINIMUM) ){
            if(MULTITOOL_PHPVERSIONMINIMUM > phpversion() ){
                ...
                if( $display == true){
                    ...
                    between PHP versions per WP installation or sometimes hosting allows customers to switch using their control panel.', 'multitool' ),MULTITOOL_PHPVERSIONMINIMUM)
                    , 'warning', 'Large', __( 'Multitool Requires PHP ', 'multitool' ) . MULTITOOL_PHPVERSIONMINIMUM);                
                }
            }
        }
    ...
    /**
    * Wrapper, uses multitool_url_toadmin to create local admin url
    * 
    * @param mixed $page
    * @param mixed $values 
    */
    ...
    */
    ...
    public function get_installed_version() {
        return get_option( 'multitool_installedversion' );    
    }  
    ...
    */
    public static function get_option_schedule_array() {
        $multitool_schedule_array = get_option( 'multitool_schedule' );
        return maybe_unserialize( $multitool_schedule_array );    
    }
    ...
    */
    public function update_option_schedule_array( $schedule_array ){
        ...
        return update_option( 'multitool_schedule', $schedule_array_serialized);    
    }
    
    public function update_settings( $multitool_settings ){
        $admin_settings_array_serialized = maybe_serialize( $multitool_settings );
        return update_option( 'multitool_settings', $admin_settings_array_serialized);    
    }
    ...
    */
    ...
    /**
    * Determines if the giving value is a Multitool page or not
    */
    public function is_plugin_page( $page){
        return strstr( $page, 'multitool' );  
    } 
    ...
    */
    public function event_return( $return_reason){
        $multitool_schedule_array = self::get_option_schedule_array();
        $multitool_schedule_array['history']['lastreturnreason'] = $return_reason;
        self::update_option_schedule_array( $multitool_schedule_array );   
    }  
    ...
    */
    ...
    public function create_localmedia_fromlocalimages( $file_url, $post_id ){           
        ...
        global $wpdb, $multitool_settings;
        ...
        if( isset( $multitool_settings['create_localmedia_fromlocalimages']['destinationdirectory'] ) ){   
            $artDir = $multitool_settings['create_localmedia_fromlocalimages']['destinationdirectory'];
        }else{
    ...
    */
    public function create_posts( $project_id, $total = 1 ){
        ...
        $autoblog = new MULTITOOL_InsertPost();
        $autoblog->settings = $multitool_settings;
    ...
    */
    public function update_posts( $project_id, $total = 1, $post_id = false, $atts = array() ){
        ...
        $autoblog = new MULTITOOL_UpdatePost();
        $autoblog->settings = $multitool_settings;
        $autoblog->maintable = $database_table;
    ...
    * @param mixed $level
    * 
    * @deprecated MULTITOOL_Categories class created
    */
...
*/
class MULTITOOL_Log_Table extends WP_List_Table {
