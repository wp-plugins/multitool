...
class MULTITOOL_UI extends MULTITOOL {     
    ...
    /**
    * Called by main MULTITOOL() class using proper hook
    * 
    ...
    */
    public function add_dashboard_widgets() {
        ...
        wp_add_dashboard_widget(
             ...
             __( 'Test Widget', 'multitool' ),// Title.
        ...
        foreach( $menu_array as $key => $section_array ) {
            ...
            if( !isset( $multitool_settings['widgetsettings'][ $section_array['name'] . 'dashboardwidgetsswitch'] ) ) {       
                continue;    
            } elseif( $multitool_settings['widgetsettings'][ $section_array['name'] . 'dashboardwidgetsswitch'] !== 'enabled' ) {    
                continue;    
            }
            ...
            if( !isset( $multitool_settings['widgetsettings'][ $section_array['name'] . 'widgetscapability'] ) ) {      
                continue;
            } elseif( !current_user_can( $multitool_settings['widgetsettings'][ $section_array['name'] . 'widgetscapability'] ) ) {    
            ...
            $view_object = MULTITOOL::load_class( 'MULTITOOL_' . ucfirst( $section_array['name'] ) . '_View', $section_array['name'] . '.php', 'views' );
    ...
    */
    ...
    /**
    * Displays a table of multitool_option records with ability to view their value or delete them
    ...
    */
    public function list_optionrecordtrace( $form = false, $size = 'Small', $optiontype = 'all' ){ 
        // first get all records that begin with multitool_
        $multitoolrecords_result = $this->DB->options_beginning_with( 'multitool_' );
        $counter = 1;?>
            
            <script language="JavaScript">
            function multitool_deleteoptions_checkboxtoggle(source) {
              checkboxes = document.getElementsByName( 'multitool_deleteoptions_array[]' );
        ...
        if( $form){
            echo '<input type="checkbox" onClick="multitool_deleteoptions_checkboxtoggle(this)" /> '. __( 'Select All Options', 'multitool' ) .'<br/>';
        }
        
        $html = '';
                        
        foreach( $multitoolrecords_result as $key => $option ){
            
            if( $form){
                $html = '<input type="checkbox" name="multitool_deleteoptions_array[]" value="'.$option.'" />';
            }
    ...
    public function screens_menuoptions( $current){
        ...
        foreach( $multitool_menu_array as $page_slug => $page_array ){ 
            foreach( $multitool_menu_array[$page_slug]['tabs'] as $whichvalue => $screen_array ){
                $selected = '';
    ...
    public function page_menuoptions( $current, $return = false ){
        ...
        foreach( $multitool_menu_array as $page_slug => $page_array ){ 
            ...
            if( $return){
                return '<option value="'.$page_slug.'" '.$selected.'>'.$multitool_menu_array[$page_slug]['viewtitle'].'</option>';
            }else{ 
                echo '<option value="'.$page_slug.'" '.$selected.'>'.$multitool_menu_array[$page_slug]['viewtitle'].'</option>';
            }
        }    
    }
    ...
    */
    public function option_switch( $title, $name, $id, $current_value = 'nocurrent123', $onlabel = 'Enabled', $offlabel = 'Disabled', $default = 'enabled' ){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $title, 'multitool' ); ?></th>
            <td>
    ...
    * @deprecated use class-forms.php      
    */
    public function option_text( $title, $name, $id, $current_value = '', $readonly = false, $class = 'multitool_inputtext', $append_text = '', $left_field = false, $right_field_content = '', $required = false, $validation = false ){
    ...
    */
    public function option_text_simple( $title, $nameandid, $current_value = '', $required = false, $validation = false ){
        $this->option_text( $title, $nameandid, $nameandid, $current_value, false, 'multitool_inputtext', '', false, '', $required, $validation );
    }
    ...
    */
    ...
    public function panel_trash_icon( $form_name ){?>
        <a href="<?php echo $form_name;?>"><img src="<?php echo MULTITOOL_IMAGES_URL . 'trash-icon.gif';?>" alt="<?php _e( 'Reset form' );?>" title="<?php _e( 'Reset form' );?>"></a><?php 
    }
    
    public function panel_video_icon( $youtubeid ){
        if( $youtubeid){
            echo '<a href="https://www.youtube.com/watch?v=' . $youtubeid . '" target="_blank"><img src="' . MULTITOOL_IMAGES_URL . 'video-icon.gif" alt="' . __( 'View video on YouTube' ) . '" title="' . __( 'View video on YouTube' ) . '"></a>';
        }
    }
    ...
    */          
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] ) ){
            foreach( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] as $option_id => $option_array ){
                ...
                if( isset( $option_array['optionurl'] ) ){
                    ...
                    $all_help_content .= ' title="' . __( 'Read More about', 'multitool' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Read More', 'multitool' ) . '</a>';      
                }
      
                if( isset( $option_array['optionvideourl'] ) ){
                    $all_help_content .= ' - <a href="' . $option_array['optionvideourl'] . '"';
                    $all_help_content .= ' title="' . __( 'Watch a video about', 'multitool' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Video', 'multitool' ) . '</a>';      
                }
    ...
    public function get_notice_array() {
        $a = get_option( 'multitool_notifications' );
       
        $multitool_notice_array = maybe_unserialize( $a);
        if(!is_array( $multitool_notice_array ) ){
            return array();    
        }
        
        // delete some expired admin notices
        if( isset( $multitool_notice_array['admin'] ) )
        {
            foreach( $multitool_notice_array['admin'] as $key => $notice){
                ...
                if( isset( $notice['created'] ) )
                {
                    ...
                    if( $projected < time() )
                    {
                        unset( $multitool_notice_array['admin'][$key] );    
                    }                                               
                }
                else
                {       
        ...
        if( isset( $multitool_notice_array['users'] ) ){
 
            foreach( $multitool_notice_array['users'] as $owner_id => $owners_notices){
                ...
                foreach( $owners_notices as $key => $notice)
                {
                    if( isset( $notice['created'] ) )
                    {
                        ...
                        if( $projected < time() )
                        {
                            unset( $multitool_notice_array['users'][$key] );    
                        }                                               
                    }
                    else
                    {
        ...
        return $multitool_notice_array;    
    }
    
    public function update_notice_array() {
        global $multitool_notice_array;      
        return update_option( 'multitool_notifications',maybe_serialize( $multitool_notice_array ) );    
    }    
    ...
    */
    public function notice_return( $type, $size, $title = false, $message = 'no message has been set', $helpurl = false, $forcewtgstyle = false ){
        ...
        if( $forcewtgstyle === false ) {
            if( isset( $multitool_settings['noticesettings']['wpcorestyle'] ) && $multitool_settings['noticesettings']['wpcorestyle'] == 'enabled' ) {
    ...
    */
    public function create_notice( $message, $type = 'info', $size = 'Small', $title = false, $sensitive = false, $helpurl = false ){
        global $multitool_notice_array, $current_user;
        ...
        if( $sensitive === false ) {   
            ...
            $multitool_notice_array['notices'][$current_user->ID][$key]['sensitive'] = false;
            $multitool_notice_array['notices'][$current_user->ID][$key]['message'] = $message;
            $multitool_notice_array['notices'][$current_user->ID][$key]['type'] = $type;
            $multitool_notice_array['notices'][$current_user->ID][$key]['size'] = $size;
            $multitool_notice_array['notices'][$current_user->ID][$key]['title'] = $title; 
            $multitool_notice_array['notices'][$current_user->ID][$key]['created'] = time();
            $multitool_notice_array['notices'][$current_user->ID][$key]['helpurl'] = $helpurl;
            ...
            $multitool_notice_array['users'][$current_user->ID][$key]['sensitive'] = false;
        ...
        return $this->update_notice_array( $multitool_notice_array );
    }
    ...
    */
    public function create_prompt() {
        ## $multitool_notice_array['prompts'][$owner][$key]['message'] = $message;    
    }
    ...
    */
    ...
    public function display_users_notices() {
        global $multitool_notice_array, $current_user, $multitool_settings;
        
        $multitool_notice_array = $this->get_notice_array();
        
        if(!isset( $multitool_notice_array['notices'][$current_user->ID] ) ){return;}
 
        foreach( $multitool_notice_array['notices'][$current_user->ID] as $key => $owners_notices){
            ...
            unset( $multitool_notice_array['notices'][$current_user->ID] );
        }
                                       
        $this->update_notice_array( $multitool_notice_array );            
    }
    ...
    * 
    * @deprecated use multitool_n_postresult as of 13th February 2013
    */
    ...
    /**
    * New Notice Function (replacing MULTITOOL_Notice::notice_depreciated() )
    ...
    */
    public function n_depreciated( $title, $mes, $style, $size, $atts = array() ){
        ...
        // this allows the notice to be printed within content where the function is called rather than within the $multitool_notice_array loop
        ...
        $multitool_notice_array = $this->persistentnotifications_array();
        
        // set next array key value
        $next_key = 0;

        // determine next array key
        if( isset( $multitool_notice_array['notifications'] ) ){    
            $next_key = MULTITOOL::get_array_nextkey( $multitool_notice_array['notifications'] );
        }    
        ...
        $multitool_notice_array['notifications'][$next_key]['message'] = $mes;
        $multitool_notice_array['notifications'][$next_key]['type'] = $style;
        $multitool_notice_array['notifications'][$next_key]['size'] = $size;
        $multitool_notice_array['notifications'][$next_key]['title'] = $title;
        $multitool_notice_array['notifications'][$next_key]['helpurl'] = $url; 
        $multitool_notice_array['notifications'][$next_key]['output'] = $output;
        $multitool_notice_array['notifications'][$next_key]['audience'] = $audience;
        $multitool_notice_array['notifications'][$next_key]['user_mes'] = $user_mes;
        $multitool_notice_array['notifications'][$next_key]['side'] = $side;
        $multitool_notice_array['notifications'][$next_key]['clickable'] = $clickable;
                          
        $this->update_persistentnotifications_array( $multitool_notice_array );
    } 
    ...
    */
    public function update_persistentnotifications_array( $notifications_array ){
        return update_option( 'multitool_notifications',maybe_serialize( $notifications_array ) );    
    }
    ...
    */
    public function notice_depreciated( $message, $type = 'success', $size = 'Extra', $title = false, $helpurl = 'www.webtechglobal.co.uk', $output_type = 'echo', $persistent = false, $clickable = false, $user_type = false ){
        ...
        if( is_admin() || $output_type == 'public' ){
            ...
            if( $helpurl != false && $helpurl != '' && $helpurl != 'http://www.multitool.com/support' ){$clickable = false;}
            ...
            if( $output_type == 'return' || $output_type == 'public' ){   
                ...
                // add new message to the notifications array
                $multitool_notice_array['notifications'][$next_key]['message'] = $message;
                $multitool_notice_array['notifications'][$next_key]['type'] = $type;
                $multitool_notice_array['notifications'][$next_key]['size'] = $size;
                $multitool_notice_array['notifications'][$next_key]['title'] = $title;
                $multitool_notice_array['notifications'][$next_key]['helpurl'] = $helpurl; 
                $multitool_notice_array['notifications'][$next_key]['clickable'] = $clickable; 
    ...
    /**                                              
    * Outputs the contents of $multitool_notice_array, used in MULTITOOL::pageheader().
    * Will hold new and none persistent notifications. May also hold persistent. 
    */
    public function output_depreciated() {
        $multitool_notice_array = self::persistentnotifications_array();    
        if( isset( $multitool_notice_array['notifications'] ) ){
                                                        
            foreach( $multitool_notice_array['notifications'] as $key => $notice){
                ...
                if(!isset( $notice['persistent'] ) || $notice['persistent'] != false ){
                    ...
                    unset( $multitool_notice_array['notifications'][$key] );
                }
            }
    ...
    */
    ...
    public function add_admin_footer_text() {
        // show admin footer message (only on pages of Multitool)
        add_filter( 'admin_footer_text', array( $this, '_admin_footer_text' ) );
    }
    
    /**
     * Add a Multitool "Thank You" message to the admin footer content
     *
     * @since 0.0.1
     *
    ...
    public function _admin_footer_text( $content ) {
        global $c2p_is_free;
        $content .= ' &bull; ' . __( 'Thank you for using <a href="http://webtechglobal.co.uk/multitool/">Multitool</a>.', 'multitool' );
        // FREE EDITIONS ONLY $content .= ' ' . sprintf( __( 'Support the plugin with your <a href="%s">donation</a>!', 'multitool' ), 'http://webtechglobal.co.uk/donate/' );
        return $content;
    } 
    ...
    * @param mixed $values - begin with & followed by values
    */
    public function linkaction( $page, $action, $title = 'Multitool admin link', $text = 'Click Here', $values = '' ){
        return '<a href="'. wp_nonce_url( admin_url() . 'admin.php?page=' . $page . '&multitoolaction=' . $action  . $values, $action ) . '" title="' . $title . '" class="button c2pbutton">' . $text . '</a>';
    }
    ...
    */
    public function register_input_validation( $input_title, $input_name, $input_id, $input_validation ) {
        ...
        $form_val_arr = get_option( 'multitool_formvalidation' );
        ...
        update_option( 'multitool_formvalidation', $form_val_arr );
    }
    ...
    */
    public function metabox_nocurrentproject() {
        ...
        the one that was active.', 'multitool' );
        echo '</p>';
    }
