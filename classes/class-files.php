...
class MULTITOOL_Files { 
    
    public function __construct() {
        $this->PHP = MULTITOOL::load_class( 'MULTITOOL_PHP', 'class-phplibrary.php', 'classes' ); # php library by Ryan R. Bayne  
    }
    ...
    */
    public function upload_error( $file_error ) {
        $upload_error_strings = array( false,
            __( "The uploaded file exceeds the <code>upload_max_filesize</code> directive in <code>php.ini</code>.", 'multitool' ),
            __( "The uploaded file exceeds the <em>MAX_FILE_SIZE</em> directive that was specified in the HTML form.", 'multitool' ),
            __( "The uploaded file was only partially uploaded.", 'multitool' ),
            __( "No file was uploaded.", 'multitool' ),
            __( "Missing a temporary folder.", 'multitool' ),
            __( "Failed to write file to disk.", 'multitool' ) );  
    ...
    */
    public function singlefile_uploader( $file_array, $uploads = false, $test_size = true, $test_type = false, $overwrite = false ) {
        $result_array = array( 'outcome' => false,
                               'error' => false, 
                               'message' => __( 'The file was uploaded.', 'multitool' ) );
        ...
        if ( $test_size && !( $file_array['size'] > 0 ) ) {
            $result_array['outcome'] = false;
            $result_array['error'] = true;
            $result_array['message'] = __( 'Your file is empty. Empty files are not permitted in this operation.', 'multitool' );
            return $result_array;            
        }
        ...
        if (! @ is_uploaded_file( $file_array['tmp_name'] ) ) {
            $result_array['outcome'] = false;
            $result_array['error'] = true;
            $result_array['message'] = __( 'Sorry your file upload failed. A temporary file could not be created on the server. This may be a temporary problem, please try again.', 'multitool' );
            return $result_array;                    
        }
        ...
        if ( $test_type ) {
            ...
            if ( ( !$type || !$ext ) && !current_user_can( 'unfiltered_upload' ) ) {
                $result_array['outcome'] = false;
                $result_array['error'] = true;
                $result_array['message'] = __( 'Your file does not meet security requirements and has not been uploaded.', 'multitool' );
                return $result_array;                
            }
        ...
        if ( false === @ move_uploaded_file( $file_array['tmp_name'], $uploads['path'] . $filename ) ) {
                $result_array['outcome'] = false;
                $result_array['error'] = true;
                $result_array['message'] = __( 'Your file could not be moved into the set directory. If a file with the same name exists in that directory an overwrite may not be possible.', 'multitool' );
                return $result_array; 
        }    
    ...
    */
    public function file_from_url( $url, $uploads = array(), $overwrite = false, $allow = false ) {
        ...
        if( !$url_is_valid ) {
            $result['message'] = __( 'URL is invalid', 'multitool' );
            return false;    
        }
        ...
        if ( false !== $uploads['error'] ) {
            $result['message'] = __( 'WordPress wp_upload_dir() returned an error.', 'multitool' );
        ...
        if( $write_result === false ) {
            ...
            if( file_exists( $uploads['path'] . $file['originalName'] ) ) {
                $result['message'] = __( 'The filename already exists and the file cannot be overwritten.', 'multitool' );
                return $result;                 
            }
            
        } elseif( $write_result === false && $rename !== true ) {
            $result['failurereason'] = __( 'File already exists and renaming is not allowed.', 'multitool' );
            return $result;                
        }
        ...
        $message = __( 'The file was transferred.', 'multitool' ); 
        
        if( $overwrite_applied ) {
            $message .= ' ' . __( 'An existing file was overwritten as requested.', 'multitool' );
        }
        
        if( $filerename_applied ) {
            $message .= ' ' . __( 'The files name had to be changed as it already exists.', 'multitool' );    
        }
    ...
    */
    public function compare_csv_files( $filepath_one, $filepath_two ) {
        ...
        can switch to the newer file if required.', 'multitool' );      
