<?php
/**
 * Members and methods for Multitool views that use post boxes (eventually all views)
 *
 * @package MULTITOOL
 * @subpackage Views
 * @author Ryan Bayne 
...
abstract class MULTITOOL_View {
    ...
    public function __construct() {
        ...
        $this->Help = $this->MULTITOOL->load_class( 'MULTITOOL_Help', 'class-help.php', 'classes' );
        $this->PHP = $this->MULTITOOL->load_class( 'MULTITOOL_PHP', 'class-phplibary.php', 'classes' );
        $this->UI = $this->MULTITOOL->load_class( 'MULTITOOL_UI', 'class-ui.php', 'classes' );
        ...
        if( isset( $this->help_array[ $page_name ] ) ){
            ...
            if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pageabout' ] ) ){
                ...
                $screen->add_help_tab( array(
                    'id' => $page_name,
                    'title' => __( 'About', 'multitool' ) . ' ' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagetitle' ],
                ...
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagereadmoreurl' ] ) && is_string( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagereadmoreurl' ] ) ){
                    $pagereadmeurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagereadmoreurl' ] . '" target="_blank">' . __( 'Documentation', 'multitool') . '</a></p>';        
                } 
                               
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagevideourl' ] ) && is_string( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagevideourl' ] ) ){
                    $pagevideourl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagevideourl' ] . '" target="_blank">' . __( 'Video', 'multitool') . '</a></p>';
                } 
                               
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagediscussurl' ] ) && is_string( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagediscussurl' ] ) ){
                    $pagediscussionurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagediscussurl' ] . '" target="_blank">' . __( 'Support', 'multitool') . '</a></p>';
                }
        
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagefaqurl' ] ) && is_string( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagefaqurl' ] ) ){
                    $pagefaqurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagefaqurl' ]  . '" target="_blank">' . __( 'FAQ', 'multitool') . '</a></p>';
                }
                                
                // help tab sidebar
                $screen->set_help_sidebar( '
                <p><strong>' . __( 'For more information:', 'multitool' ) . '</strong></p>' . $pagereadmeurl . $pagevideourl . $pagediscussionurl . $pagefaqurl );
    ...
    */
    public function setup( $action, array $data ) {
        ...
        $this->MULTITOOL->enqueue_script( 'common', array( 'jquery', 'postbox' ), array() );
    ...
    */
    protected function add_text_box( $id, $callback, $context = 'normal', $wrap = false ) {
        ...
        $long_id = "multitool_{$this->action}-{$id}";
    ...
    */
    protected function action_nonce_field( array $data, array $box ) {
        wp_nonce_field( $this->MULTITOOL->nonce( $this->action ) ); echo "\n";
    }
    ...
    */
    protected function action_field( array $data, array $box ) {
        echo "<input type=\"hidden\" name=\"action\" value=\"multitool_{$this->action}\" />\n";
    }
    ...
    */       
    public function render() { 
        ...
        // get the admin page name (slug in menu array, without prepend "multitool_")
        $admin_page = $this->MULTITOOL->get_admin_page_name();
        
        // if we are on the main page change $admin_page to 'main' as that is what we use in array
        if( $admin_page === 'multitool' ){
            $admin_page = 'main';
        }   
         
        // view header - includes notices output and some admin side automation such as conflict prevention
        $this->MULTITOOL->pageheader( $multitool_menu_array[ $admin_page ]['title'], 0);
                               
        // create tab menu for the giving page if the section has two or more pages
        if( !isset( $multitool_menu_array[ $admin_page ]['tabmenu'] ) || $multitool_menu_array[ $admin_page ]['tabmenu'] === true ) {
            if( $admin_page !== 'main' ) {
                $this->MULTITOOL->build_tab_menu( $admin_page );
            }
        }
    ...
    */
    protected function textbox_submit_button( array $data, array $box ) {
        $caption = isset( $data['submit_button_caption'] ) ? $data['submit_button_caption'] : __( 'Save Changes', 'multitool' );
        ?>
