...
add_action( 'init', 'multitool_init_posttype_contentdesigns' );
add_action( 'add_meta_boxes', 'multitool_add_meta_boxes_post' );
add_action( 'save_post', 'multitool_save_meta_boxes_post',10,2 );
/**
* Must be called using add_action( 'init', 'multitool_register_customposttype_contentdesigns' )
* Registers custom post type for content only
*/
function multitool_init_posttype_contentdesigns() {
    $labels = array(
        'name' => _x( 'Multitool Content Templates', 'post type general name' ),
        'singular_name' => _x( 'Multitool Content Template', 'post type singular name' ),
...
*/
function multitool_add_meta_boxes_post() {
    ...
    //if( $multitool_settings['monetizesection']['adsense']['authoradsense']['switch'] == 'enabled' && current_user_can( 'publish_posts' ) ){
    //    add_meta_box(
    //        'posts-meta-authoradsense',
    //        esc_html__( 'Author AdSense' ),
    //        'multitool_metabox_authoradsense',
    //        'post',
    //        'side',
    //        'default'
    //    );
...
*/
function multitool_metabox_authoradsense( $object, $box ) { 
    //wp_nonce_field( basename( __FILE__ ), 'postnonce' );
    //$value = get_post_meta( $object->ID, '_multitool_authoradsense', true );
    //echo '<textarea id="c2pauthoradsense" name="multitool_post_authoradsense" rows="5" cols="30">'.$value.'</textarea>'; 
} 

/**
* Save meta box's for post posts type
*/
function multitool_save_meta_boxes_post( $post_id, $post ) {    
    ...
    {  
        ...
        /* Get the meta key. */
        $meta_key = '_multitool_' . $term;
                                            
        if( isset( $_POST['multitool_post_'.$term] ) )
        {
            $new_meta_value = $_POST['multitool_post_'.$term];    
        }
